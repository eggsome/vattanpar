...
GUI editor for the crateblast map format. Draw polygonal walls and terrain,
place crates and the spawn point, save back to the same plain-text format
the game reads.

Usage:  python3 editor.py [map.txt]

Controls:
  Tool buttons     select / draw wall / draw terrain / place crate / spawn
...
KIND_COLORS = {  # matches src/draw.c
    ...
    "wall":  (0x26 / 255, 0x29 / 255, 0x31 / 255),
}
CRATE_COLOR = (0xB4 / 255, 0x71 / 255, 0x2F / 255)
GRID = 160
SNAP = 20
CRATE_SIZE = 44
PLAYER_SIZE = 36
        ...
        for line in f:
            ...
            try:
                if tok[0] == "size":
                    ...
                    m["walls"].append(rect_pts(*map(float, tok[1:5])))
                elif tok[0] == "terrain":
                    m["terrains"].append(
...
def shape_line(kind, pts):
    r = as_axis_rect(pts)
    if r is not None:
        head = "wall" if kind == "wall" else f"terrain {kind}"
...
def save_map(path, m):
    ...
    out += [shape_line("wall", pts) for pts in m["walls"]]
    out.append("")
...
class EditorWindow(Gtk.ApplicationWindow):
    ...
    def build_ui(self):
        ...
        first = None
        self.tool_buttons = {}
        for name in ["select", "wall"] + TERRAIN_KINDS + ["crate", "spawn"]:
    ...
    def finish_draft(self):
        if len(self.draft) >= 3:
            self.push_undo()
            if self.tool == "wall":
                self.map["walls"].append(self.draft)
    ...
    def select_press(self, x, y):
        ...
        for i in reversed(range(len(self.map["crates"]))):
            ...
            if abs(cx - wx) < CRATE_SIZE / 2 and abs(cy - wy) < CRATE_SIZE / 2:
                start_move(("crate", i), self.map["crates"][i])
                return
        sx, sy = self.map["spawn"]
        if abs(sx - wx) < PLAYER_SIZE / 2 and abs(sy - wy) < PLAYER_SIZE / 2:
            start_move(("spawn", 0), self.map["spawn"])
            return
        for i in reversed(range(len(self.map["walls"]))):
            if point_in_poly(self.map["walls"][i], wx, wy):
                start_move(("wall", i), list(self.map["walls"][i]))
                return
    ...
    def on_draw(self, area, cr, w, h):
        ...
        for pts in m["walls"]:
            self.poly_path(cr, pts)
            cr.set_source_rgb(*KIND_COLORS["wall"])
            cr.fill_preserve()
            cr.set_source_rgb(0.24, 0.25, 0.31)
            cr.set_line_width(2)
            cr.stroke()

        half = CRATE_SIZE / 2 * self.zoom
        for cx, cy in m["crates"]:
            sx, sy = self.w2s(cx, cy)
            cr.rectangle(sx - half, sy - half, half * 2, half * 2)
            cr.set_source_rgb(*CRATE_COLOR)
            cr.fill_preserve()
        ...
        ph = PLAYER_SIZE / 2 * self.zoom
        cr.rectangle(sx - ph, sy - ph, ph * 2, ph * 2)
        ...
        if self.sel and self.sel[0] == "crate":
            ...
            cr.rectangle(dx - half - 3, dy - half - 3,
                         half * 2 + 6, half * 2 + 6)
